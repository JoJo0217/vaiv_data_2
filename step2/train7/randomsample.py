...
with open('dpo_koultra_feedback.jsonl', "r", encoding="utf-8") as file:
    for line in file:
        result.append( json.loads(line))
...
random_indices = random.sample(range(len(data)), int(len(data)/2))
...
with jsonlines.open("dpo_koultra_feedback_1.jsonl", mode='w') as writer:
...
with jsonlines.open("dpo_koultra_feedback_2.jsonl", mode='w') as writer:
